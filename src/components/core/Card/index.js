            ...
            <div data-aos="zoom-in-up" data-aos-once="true" key={i} className={`item${i} col p-5 px-3 md:px-5 w-[100%] sm:w-[100%] md:w-[100%] lg:w-[100%] xl:w-[80%] m-auto`}>
                <div className="team-content relative h-full border-4 border-gray-300 border-opacity-60 rounded-2xl overflow-hidden">
                    ...
                    <div className="name-card p-6 bg-white hover:bg-gradient-to-r hover:text-white z-10 w-full hover:from-red-500 hover:to-red-700 transition duration-300 ease-in">
                        <h2 className="sub-text text-base sm:text-sm 2xl:text-lg font-medium text-indigo-300 mb-1 h-[35px]">{item.subText}</h2>
            ...
            <div className="container px-5 py-12 mx-auto">
                ...
                {props.data.length > props.noOfItems && props.noOfItems ? (
                    <div className='see-more-box hidden'>
                        <button onClick={props.showAllList} className='see-more-btn text-sm font-semibold mt-10 px-10 py-3 mb-10 rounded-2xl text-white border-2 border-white hover:text-black hover:bg-white hover:border-2 hover:border-black transition duration-300 ease-in uppercase'>See More</button>
                    </div>
                ) : null}
            </div>
        </div>
